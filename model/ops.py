...
#    return tf.compat.v1.layers.batch_normalization(x, decay=decay, updates_collections=None, epsilon=epsilon,
#                                        scale=True, is_training=is_training, scope=scope)
    return tf.compat.v1.layers.batch_normalization(x, epsilon=epsilon, scale=True, is_training=is_training)
...
def deconv2d(x, output_shape, kh=5, kw=5, sh=2, sw=2, stddev=0.02, scope="deconv2d"):
    with tf.compat.v1.variable_scope(scope):
        ...
        biases = tf.compat.v1.get_variable('b', [output_shape[-1]], initializer=tf.constant_initializer(0.0))
...
def conditional_instance_norm(x, ids, labels_num, mixed=False, scope="conditional_instance_norm"):
    with tf.compat.v1.variable_scope(scope):
        ...
        scale = tf.compat.v1.get_variable("scale", [labels_num, output_filters], tf.float32, tf.constant_initializer(1.0))
        shift = tf.compat.v1.get_variable("shift", [labels_num, output_filters], tf.float32, tf.constant_initializer(0.0))
